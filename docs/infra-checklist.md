...
- [ ] **Cron (reset planera Free):** w Vercel ustaw `CRON_SECRET` (ten sam sekret, który weryfikuje `GET /api/cron/free-planner-reset`). Plik `vercel.json` rejestruje zadanie tygodniowe; po deploy sprawdź w Vercel → Crons, czy job jest aktywny.
