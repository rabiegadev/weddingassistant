...
export async function GET(req: NextRequest) {
  ...
  if (!raw) {
    return NextResponse.redirect(new URL("/logowanie?e=brak+tokenu", origin));
  }
  ...
  if (!t) {
    return NextResponse.redirect(new URL("/logowanie?e=link+niepoprawny", origin));
  }
  ...
  return NextResponse.redirect(new URL("/logowanie?k=client&w=1", origin));
}
